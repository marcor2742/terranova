	...
	uploadFile(file: File, uploadUrl: string): Observable<UploadResponse> {
		...
		return this.http.request(req).pipe(
			...
			map((event: HttpResponse<any>) => {
				...
				return {
					error: 'Upload successful, but the response format was unexpected.',
				};
			}),
			catchError((errorResponse: HttpErrorResponse) => {
				...
				if (errorResponse.error instanceof ErrorEvent) {
					...
					if (
						...
						errorResponse.error.length < 250 &&
						errorResponse.error.indexOf('<') === -1
					) {
	...
	downloadFile(fileUrl: string): void {
		if (isPlatformBrowser(this.platformId)) {
			...
			if (
				...
				console.error(
					'Download attempt with invalid or missing URL:',
					fileUrl
				);
			}
		} else {
			console.warn(
				'FileManagementService.downloadFile called on the server. Download operation skipped.'
			);
		}
	}

	uploadImage(formData: FormData, url: string): Observable<UploadResponse> {
